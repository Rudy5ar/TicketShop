...
public class ManifestationService {
    ...
    public ManifestationService(ManifestationRepository manifestationRepository, LocationRepository locationRepository, UserRepository userRepository) {
    ...
    public Manifestation getManifestation(Long id) {
        return manifestationRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Manifestation not found"));
    }

    @Transactional
    public Manifestation updateManifestation(Manifestation manifestation, Long id) {
        manifestationRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Manifestation not found"));
        return manifestationRepository.save(manifestation);
    }
    ...
    public Manifestation sellerAddManifestation(Manifestation manifestation, Long locationId, Long userId) {
        if (manifestationRepository.findByLocationAndDateBetween(
                manifestation.getLocation(), manifestation.getDate(), manifestation.getDate().plusHours(6)).isEmpty()) {

            manifestation.setLocation(locationRepository.findById(locationId).orElseThrow(() -> new EntityNotFoundException("Location for manifestation not found")));
    ...
    @Transactional
    public Manifestation approveManifestation(Long manifestationId) {
        Manifestation manifestation = manifestationRepository.findById(manifestationId).orElseThrow(() -> new EntityNotFoundException("Manifestation not found"));
